...
god_mode_enabled = False  # Default god mode to off
pet_birth_time_loaded = None  # Track if we loaded a birth time
...
try:
    ...
    if pet_settings and 'god_mode' in pet_settings:
        god_mode_enabled = pet_settings['god_mode'].lower() == 'true'
        print(f"God mode: {god_mode_enabled}")
    
    # Load pet birth time if it exists
    if pet_settings and 'pet_birth_time' in pet_settings:
        try:
            pet_birth_time_loaded = float(pet_settings['pet_birth_time'])
            print(f"Loaded pet birth time: {pet_birth_time_loaded}")
        except:
            print("Could not parse pet birth time")
...
gamestate.states["pet_birth_time"] = pet_birth_time_loaded if pet_birth_time_loaded else time()  # Use loaded time or current time
gamestate.states["god_mode"] = god_mode_enabled  # Store god mode setting
...
# Game Variables
DEBUG_MODE = False  # Set to True to enable debug logging (prints game state every frame)
...
def draw_clock_and_age(display):
    ...
    # Safeguard: If age is negative (time was changed), reset to 0
    if age_days < 0:
        print(f"Warning: Negative age detected ({age_days:.1f}). Resetting birth time.")
        gamestate.states["pet_birth_time"] = current_seconds
        age_days = 0
...
def do_toolbar_stuff():
    ...
    if tb.selected_item == "clock":
        ...
        if pet_settings:
            ...
            # Only use the birth time from settings if it exists, otherwise keep current
            if 'pet_birth_time' in pet_settings and pet_settings['pet_birth_time'] is not None:
                gamestate.states["pet_birth_time"] = pet_settings['pet_birth_time']
            else:
                gamestate.states["pet_birth_time"] = current_birth_time
            
            # Update god mode if present
            if 'god_mode' in pet_settings:
                gamestate.states["god_mode"] = pet_settings['god_mode']
                god_status = "On" if pet_settings['god_mode'] else "Off"
                heart_status.message = f"God Mode: {god_status}"
                heart_status.popup(oled)
                clear()
            ...
            # Save all settings including birth time and god mode
            settings_obj = settings.Settings(i2c, oled)
            settings_obj.save_settings(
                pet_birth_time=gamestate.states["pet_birth_time"],
                god_mode=gamestate.states.get("god_mode", False)
            )
    if tb.selected_item == "heart":
        # Wake up if sleeping
        force_wake_up()
...
def check_alerts():
    ...
    # ONLY trigger death if god mode is disabled
    if not gamestate.states.get("god_mode", False):
        ...
        # In god mode, prevent stats from going below 1 to avoid death
        if gamestate.states["health"] <= 0:
...
def draw_network_menu():
    ...
    # Menu options
    options = ["Host (Wait)", "Visit (Find)", "Web Remote", "Cancel"]
    
    start_y = 22
    for i, option in enumerate(options):
        # Highlight selected option
        if i == gamestate.states["network_menu_selection"]:
            oled.text(f">{option}", 0, start_y + i * 10)
        else:
            oled.text(f" {option}", 0, start_y + i * 10)
...
def handle_network_menu_input():
    ...
    if button_a.is_pressed:
        # Move selection up
        gamestate.states["network_menu_selection"] = (gamestate.states["network_menu_selection"] - 1) % 4
        draw_network_menu()
        return True
        ...
        if selection == 0:  # Host
            ...
            start_visit_mode()
        elif selection == 2:  # Web Remote
            # Import web mode module only when needed (saves memory)
            try:
                import web_mode
                web_mode.start_web_mode(oled, gamestate, connect_wifi_improved)
            except Exception as e:
                ...
                oled.show()
                sleep(2)
        # selection == 3 is Cancel, just exit
...
while True:
    ...
    if (gamestate.states["visit_menu_active"] or 
        ...
        # Update web mode if active (only imports module if web mode was started)
        if gamestate.states.get("web_mode_active", False):
            try:
                import web_mode
                web_mode.update_web_mode(oled, gamestate, button_x, baby, baby_left, baby_center, baby_right,
                ...
                pass  # Silently fail if web mode module not available
